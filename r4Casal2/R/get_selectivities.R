...
"get_selectivities.casal2TAB" <- function(model, reformat_labels = TRUE) {
  ...
  for (i in 1:length(model)) {
    ...
    sel_molten <- suppressMessages({
      melt((sel_df), variable.name = "colname", value.name = "selectivity", factorsAsStrings = F)
    })
    bin_labs <- unlist(lapply(strsplit(as.character(sel_molten$colname), split = ".", fixed = T), FUN = function(x) {
      x[2]
    }))
    selectivity_lab <- unlist(lapply(strsplit(as.character(sel_molten$colname), split = ".", fixed = T), FUN = function(x) {
      x[1]
    }))
    ## cut out selectivity
    selectivity_lab <- unlist(lapply(strsplit(selectivity_lab, split = "[", fixed = T), FUN = function(x) {
      x[2]
    }))
