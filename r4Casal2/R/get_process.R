#' @title get_process
#' @description An accessor function that returns a data frame from a Casal2 model output of a nameed process
...
#'
"get_process" <- function(model, label, ...) {
  UseMethod("get_process", model, label)
}

#' @rdname get_process
#' @method get_process casal2MPD
#' @export
"get_process.casal2MPD" <- function(model, label, reformat_labels = TRUE) {
  if (reformat_labels) {
