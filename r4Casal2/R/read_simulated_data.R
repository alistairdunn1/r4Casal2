...
#' @description A utility function for reading in Casal2 simulated data sets. For help on simulated data from Casal2 see
#' @author Craig Marsh
...
#' @param verbose true print out statements thoughout function to help debug
...
read_simulated_data <- function(dir, verbose = FALSE, mean_age = TRUE, model_type = "age") {
  ...
  for (n in 1:n_obs) {
    ...
    for (i in 1:length(extensions)) {
      cas_sim_obs <- tryCatch(expr = extract.csl2.file(file = paste0(sim_file_names[n], ".", extensions[i]), path = dir, quiet = T), error = function(e) {
        e
      }, warning = function(w) {
        w
      })
